...
for await (const line of input.readLines()) {
    const row = line.split('').map(char => ({
        char,
        isStart: char === 'E',
        visited: char === 'E' ? true : false,
        distance: char === 'E' ? 0 : Infinity,
        previousNode: undefined,
        neighbours: []
    }))
    map.push(row)
...
const alphabet = "SabcdefghijklmnopqrstuvwxyzE".split('').reverse().join('')
const calculateDistance = (a, b) => Math.abs(alphabet.indexOf(b) - alphabet.indexOf(a) + 1)
...
while (queue.length > 0) {
    ...
    currentClosest = queue.shift()

    currentClosest.visited = true

    if (currentClosest.char === 'a' ||  currentClosest.char === 'S') {
        break
    }
